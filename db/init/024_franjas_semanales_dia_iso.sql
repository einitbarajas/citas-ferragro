-- Franjas semanales por día ISO (1=lunes … 7=domingo). Idempotente.

ALTER TABLE "FranjasPermitidasCita"
  ADD COLUMN IF NOT EXISTS "DiaSemanaIso" SMALLINT;

UPDATE "FranjasPermitidasCita"
SET "DiaSemanaIso" = 1
WHERE "DiaSemanaIso" IS NULL;
...
WHERE f."DiaSemanaIso" = 1
  AND NOT EXISTS (
    SELECT 1
    FROM "FranjasPermitidasCita" x
    WHERE x."IdBodega" = f."IdBodega"
      AND x."Orden" = f."Orden"
      AND x."HoraInicio" = f."HoraInicio"
      AND x."HoraFin" = f."HoraFin"
      AND x."DiaSemanaIso" = d."DiaSemanaIso"
      AND (
        (x."IdEquipoDescargaBodega" IS NULL AND f."IdEquipoDescargaBodega" IS NULL)
        OR x."IdEquipoDescargaBodega" = f."IdEquipoDescargaBodega"
      )
  );

ALTER TABLE "FranjasPermitidasCita"
  ALTER COLUMN "DiaSemanaIso" SET NOT NULL;

ALTER TABLE "FranjasPermitidasCita"
  DROP CONSTRAINT IF EXISTS "ChkFranjaDiaSemanaIso";

ALTER TABLE "FranjasPermitidasCita"
  ADD CONSTRAINT "ChkFranjaDiaSemanaIso" CHECK ("DiaSemanaIso" BETWEEN 1 AND 7);
...
COMMENT ON COLUMN "FranjasPermitidasCita"."DiaSemanaIso" IS 'Día ISO 8601: 1=lunes … 7=domingo. Mismas horas por día configurado.';
