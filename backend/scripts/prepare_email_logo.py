#!/usr/bin/env python3
"""Genera un único logo PNG transparente (correo y portal)."""
...
def main() -> None:
    ...
    logo = remove_outer_background(Image.open(src))
    for name in ("ferragro-logo-email.png", "ferragro-logo.png"):
        logo.save(STATIC / name, "PNG", optimize=True)
        logo.save(PUBLIC / name, "PNG", optimize=True)
    logo.save(STATIC / "ferragro-logo-transparent.png", "PNG", optimize=True)

    print(f"source={src.name} -> transparent {logo.size} (RGBA)")
